...
2. **Code review and collaboration:** Exchange revisions between a student and PI, or among members of a lab, without emailing files named `analysis_final_v2_revised.R`.
3. **Sharing and reuse:** Make a well-documented repository public as part of your professional portfolio, share the complete analysis associated with a paper, or allow another researcher to fork the repository and adapt the code for a new project.
...
| `R/` | R scripts and functions, optionally numbered in workflow order, such as `01_import_clean.R` and `02_analysis.R` | Yes |
...
This example uses `Data Files`, `Figures`, and `Tables` as folder names. Lowercase names without spaces, such as `data`, `figures`, and `tables`, are also common and slightly easier to type in code. Whichever style you choose, use it consistently.
...
In `Data Files/README.md`, document:
...
| Quarto and R Markdown source files (`.qmd`, `.Rmd`) | `.RData`, `.Rhistory`, and temporary files |
...
For this simple workflow, both people may work directly on `main`, but they should coordinate the handoff and avoid editing the same files at the same time. For larger groups, simultaneous work, or more formal review, use separate branches and GitHub pull requests rather than having everyone edit `main` directly.
